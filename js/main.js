...
var System = function() {
  ...
  //System will encompass nearly all objects and methods, avoiding global namespace cluttering.
  ...
  this.menu[this.menu.data.item1] = function(e){

    console.log("Starting Game");
    system.game.state = 1;

    system.screen.ctx.fillStyle="#000";
    system.screen.ctx.fillRect(0,0, system.screen.canvas.width, system.screen.canvas.height);

    system.Modules.gameMenu.close();
  };
  this.menu[this.menu.data.item2] = function(e){
    console.log("Saving and loading not implemented yet.");
  };
  this.menu[this.menu.data.item3] = function(e){
    console.log("No settings added yet.");
  };
  this.menu[this.menu.data.item4] = function(e){
  ...
  this.game.states.menu = function(){
    ...
    // assign canvas to system.
    // -------- Provisionable, as *can* be done at $(document).ready
    system.screen.canvas = document.getElementById("screen");
    system.screen.ctx = system.screen.canvas.getContext('2d');

    $('a#menu_' + system.menu.data.item1).click(system.menu[system.menu.data.item1]);
    $('a#menu_' + system.menu.data.item2).click(system.menu[system.menu.data.item2]);
    $('a#menu_' + system.menu.data.item3).click(system.menu[system.menu.data.item3]);
    $('a#menu_' + system.menu.data.item4).click(system.menu[system.menu.data.item4]);


  };

  /* 1 */ this.game.states.test = function(){};
